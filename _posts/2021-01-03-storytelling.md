...
<img src="/assets/images/Block%20Structure.png" alt="Figure 1: Block Structure" class="center">
...
<img src="/assets/images/StorytellingFig01.png" alt="Figure 2: First exchange with the bot" class="center">
...
<img src="/assets/images/StorytellingFig02.png" alt="Figure 3: Bot personas" class="center">
...
We eventually succeeded in implementing the first two steps. Below, the implementation approach, as well as some challenges, are explained in more detail.
...
We completed the implementation of the first two steps because we deem these most important for the replayability of the game and a more fair experience within the storyline for the users. In particular, in the previous story pipeline, if players succeeded with all tasks and then, for instance, failed the last one, they lost all their progress and immediately failed their escape mission. Such an abrupt end of the game might demotivate learners, so they wouldn’t try to restart the escape. Yet, allowing an infinite number of retries for the tasks wouldn't incentivice the users to try their best. Therefore, we decided to only allow one repitition per task type.

Several individual parts implement the repeatability functionality: first, the newly created attribute `_reps_counter` of the `RoomManager` class counts the number of times each mandatory task type was played. After one of those tasks (i.e., discussion, sentence correction, or vocabulary guessing task) ends, `_reps_counter` is increased. If the respective task fails, another counter attribute `_current_task_count` is decreased by one, essentially resetting the players’ progress through the storyline to the state before attempting the task. Secondly, if the maximum number of allowed repetitions of the respective task is not surpassed yet (i.e., is smaller than two), the task is added to the set of available tasks for the next round (`_available_tasks`). Each task handler (`discussion_handler.py`, `sen_corr_handler.py`, `vocab_desc_handler.py`) checks itself if the task was finished by retrieving the mentioned attributes of the `RoomManager` and possibly mutating them.
...
Implementing other storyline decision points on top of the repeatability was outside the scope for the final semester because these different storylines would depend on individual user and group-level scores per task. As these require implementing novel task completion metrics, coding these would require a major restructuring of the database, the task classes and handlers. Therefore, we consider the completed implementation of the task repeatability a good compromise and improvement of the current game flow.

<div id="outlook_more"></div>

## Outlook to more storytelling improvements

There is still room for improvement in the story pipeline, which would make the game more engaging. To apply the different story branches, we first calculate the score of the whole group and divide it into 3 categories. To categorize the score, we calculate how many mistakes does the group make in total and normalize that score across 1 to 4. Given score 1 out of 4, we provide story branch one. With score 2 or 3 out of 4, we go to branch 2 of the story. And finally with score 4 out of 4 we follow the story branch 3.

In addition to this splitting and choosing of the story pipeline in between each task, we also had the idea to give a different ending at the end of the game depending on the player choices and total scores. We accumulate the scores of each task, which are normalized to the highest score of 4 and take the mean of all tasks completed, including the listening task (if the group decided to complete that task as well). As we will also have an option of deciding to Escape with “Elias” the friendly alien or without him, we can first divide the story into 2 parts. And after that – depending on the total mean score – we can divide the story ending to 3 parts, giving us a total of 6 different story endings.

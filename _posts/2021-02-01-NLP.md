...
As the chatbot is intended for an intermediate English language learner, we are focusing on grammatical errors common to that proficiency level when providing final qualitative feedback. More specifically, we defined a number of common errors that are reliably identified by our [tools](#tools-and-models) as described below, and provide some general feedback about the underlying grammatical rules, if specified conditions are met (see [implementation](#implementation) for details). We narrowed down the list of common errors that we used to test our tools (as described below) to five error types that we deem most important and that are identified correctly by our tools (see [LanguageTool](#languagetool) for more details). These errors are: forgetting a comma in compound sentences, incorrectly using _who_ vs. _whom_, incorrectly using _a_ vs. _an_, making mistakes in subject-verb agreement, and using a main verb in the past tense when _did_ is used in the sentence.

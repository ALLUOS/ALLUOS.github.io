...
<div id="path"></div>
## Semester Three - Adaptive module weaknesses led to player self-regulation approach
...
### Reworking of the adaptive module - motivation

The original adaptive module consisted of two main parts: the adaptivity of task iterations based on a learners’ language proficiency model (todo: link) and the self-regulatory manual selection of the difficulty by the players themselves (todo: link). 

In the previous semester, we noted several shortcomings of the existing module. Firstly, we think the game has low re-playability value. Our gamification settings of an escape game imply limited interest in repeated engagement with the bot – a solution of providing a variety to the storyline has been designed, and more information can be found here (todo: link to design from 3. Semester) and here (todo: implementation of the new story pipeline), however, the task data also did not contain enough variety to keep providing novel educational content over a long period of time. As such, the developed learner model would likely not receive enough user data in order to work appropriately as designed. 

Secondly, as pointed out by the previous collaborators themselves, the data difficulty was labelled somewhat arbitrarily, and although there has been an attempt at mitigating this by introducing user self-regulation, since the adaptive features were not transparent to the users, the option to choose paths lead to confusion and was eventually disabled. We therefore felt that the overall adaptivity level of the current version was rather low. 

Lastly, although the players may have been motivated to play the game again in case of an unsuccessful first try at completing the escape mission, they did not receive sufficient feedback on their mistakes that would actually help them overcome their failures in future attempts.
...
We therefore considered several directions, on which adaptive features could be implemented into the existing structure of the bot/escape mission:
...
To some extent, similar guidance had already been implemented into the discussion task in the previous semester by providing the users with some general feedback and tips regarding their grammar (more details can be found here – todo: link GECTOR). The newest improvements have therefore been done to the sentence correction and vocabulary tasks, as described in more detail below. The remaining listening task is optional and therefore left out from further work on adaptivity. 
